...
class OptionDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('product.store.name', function ($option) {
                return getLinksColumnByRouteName([$option->product->store->toArray()], 'stores.edit', 'id', 'name');
            })
    ...
    public function query(Option $model)
    {
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("product")->with("optionGroup")->with('product.store');
