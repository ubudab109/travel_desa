...
<!-- Id Field -->
<div class="form-group row col-md-4 col-sm-12">
    {!! Form::label('store', trans('lang.store'), ['class' => 'col-4 control-label']) !!}
    <div class="col-8">
        @if(isset($order->productOrders[0]))
            <p>{!! $order->productOrders[0]->product->store->name !!}</p>
        @endif
    </div>

    {!! Form::label('store_address', trans('lang.store_address'), ['class' => 'col-4 control-label']) !!}
    <div class="col-8">
        @if(isset($order->productOrders[0]))
            <p>{!! $order->productOrders[0]->product->store->address !!}</p>
        @endif
    </div>

    {!! Form::label('store_phone', trans('lang.store_phone'), ['class' => 'col-4 control-label']) !!}
    <div class="col-8">
        @if(isset($order->productOrders[0]))
            <p>{!! $order->productOrders[0]->product->store->phone !!}</p>
        @endif
    </div>
